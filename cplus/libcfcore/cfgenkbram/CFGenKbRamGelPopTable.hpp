...
// Description: C++18 Interface for an in-memory RAM DbIO declementation for GelPop.
...
namespace cfcore {
	...
	class CFGenKbRamGelPopTable
	: public virtual cfcore::ICFGenKbGelPopTable
	{
	protected:
		cfcore::ICFGenKbSchema* schema;
		std::map<cfcore::CFGenKbGelInstructionPKey,
			cfcore::CFGenKbGelPopBuff*>* dictByPKey;
		std::map<cfcore::CFGenKbGelPopByRemainderIdxKey,
			std::map<cfcore::CFGenKbGelInstructionPKey,
				cfcore::CFGenKbGelPopBuff*>*>* dictByRemainderIdx;
	public:
		static const std::string CLASS_NAME;
		CFGenKbRamGelPopTable( cfcore::ICFGenKbSchema* argSchema );

		virtual ~CFGenKbRamGelPopTable();
		...
		virtual std::TCFLibOwningVector<cfcore::CFGenKbGelPopBuff*> readAllDerived( const cfcore::CFGenKbAuthorization* Authorization );
		...
		virtual std::TCFLibOwningVector<cfcore::CFGenKbGelPopBuff*> readAllBuff( const cfcore::CFGenKbAuthorization* Authorization );
		...
		virtual std::TCFLibOwningVector<cfcore::CFGenKbGelPopBuff*> pageBuffByRemainderIdx( const cfcore::CFGenKbAuthorization* Authorization,
			const int64_t* RemainderTenantId,
			const int64_t RemainderGelCacheId,
			const int64_t* RemainderInstId,
