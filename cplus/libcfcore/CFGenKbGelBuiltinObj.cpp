// Description: C++18 base object instance implementation for CFGenKb GelBuiltin.
...
namespace cfcore {
	...
	const std::string CFGenKbGelBuiltinObj::CLASS_NAME( "CFGenKbGelBuiltinObj" );
	const classcode_t CFGenKbGelBuiltinObj::CLASS_CODE = 0xa92bL;


	CFGenKbGelBuiltinObj::CFGenKbGelBuiltinObj()
	: ICFGenKbGelBuiltinObj(),
	  CFGenKbGelInstructionObj()
	{
	}

	CFGenKbGelBuiltinObj::CFGenKbGelBuiltinObj( cfcore::ICFGenKbSchemaObj* argSchema )
	: ICFGenKbGelBuiltinObj(),
	...
	cfcore::CFGenKbGelInstructionBuff* CFGenKbGelBuiltinObj::getBuff() {
		if( buff == NULL ) {
			if( isNew ) {
				buff = new cfcore::CFGenKbGelBuiltinBuff();
				copyBuffToPKey();
			}
			else {
				// Read the data buff via the backing store
				buff = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelBuiltin()->readDerivedByPIdx( dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getAuthorization(),
	...
	void CFGenKbGelBuiltinObj::setBuff( cfcore::CFGenKbGelInstructionBuff* value ) {
	...
	const std::string& CFGenKbGelBuiltinObj::getClassName() const {
		return( CLASS_NAME );
	}

	const classcode_t CFGenKbGelBuiltinObj::getClassCode() const {
		return( CLASS_CODE );
	}

	bool CFGenKbGelBuiltinObj::implementsClassCode( const classcode_t value ) const {
	...
	std::string CFGenKbGelBuiltinObj::toString() {
		static const std::string S_Space( " " );
		static const std::string S_Preamble( "<CFGenKbGelBuiltinObj" );
	...
	std::string CFGenKbGelBuiltinObj::getObjName() {
	...
	const std::string CFGenKbGelBuiltinObj::getGenDefName() {
		return( cfcore::CFGenKbGelBuiltinBuff::GENDEFNAME );
	}

	cflib::ICFLibAnyObj* CFGenKbGelBuiltinObj::getObjQualifier( const classcode_t* qualifyingClass ) {
	...
	cflib::ICFLibAnyObj* CFGenKbGelBuiltinObj::getNamedObject( const classcode_t* qualifyingClass, const std::string& objName ) {
	...
	cflib::ICFLibAnyObj* CFGenKbGelBuiltinObj::getNamedObject( const std::string& objName ) {
		std::string nextName;
	...
	std::string CFGenKbGelBuiltinObj::getObjQualifiedName() {
	...
	std::string CFGenKbGelBuiltinObj::getObjFullName() {
		const static std::string S_Dot( "." );
	...
	cfcore::ICFGenKbGelInstructionObj* CFGenKbGelBuiltinObj::realize() {
		cfcore::ICFGenKbGelBuiltinObj* retobj = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGelBuiltinTableObj()->realizeGelBuiltin(
			(cfcore::ICFGenKbGelBuiltinObj*)this );
		return( dynamic_cast<cfcore::ICFGenKbGelInstructionObj*>( retobj ) );
	}

	cfcore::ICFGenKbGelInstructionObj* CFGenKbGelBuiltinObj::read( bool forceRead ) {
		cfcore::ICFGenKbGelBuiltinObj* retobj = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGelBuiltinTableObj()->readGelBuiltinByPIdx( getGelBuiltinBuff()->getRequiredTenantId(),
		getGelBuiltinBuff()->getRequiredGelCacheId(),
		getGelBuiltinBuff()->getRequiredGelInstId(), forceRead );
		return( dynamic_cast<cfcore::ICFGenKbGelInstructionObj*>( retobj ) );
	}

	cfcore::ICFGenKbGelInstructionEditObj* CFGenKbGelBuiltinObj::beginEdit() {
		...
		if( ! getIsNew() ) {
			cfcore::ICFGenKbGelBuiltinObj* lockobj = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGelBuiltinTableObj()->lockGelBuiltin( getPKey() );
			lockobj->realize();
		}
		edit = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGelBuiltinTableObj()->newEditInstance( this );
		return( dynamic_cast<cfcore::ICFGenKbGelInstructionEditObj*>( edit ) );
	}

	cfcore::ICFGenKbGelBuiltinEditObj* CFGenKbGelBuiltinObj::getGelBuiltinEdit() {
		return( dynamic_cast<cfcore::ICFGenKbGelBuiltinEditObj*>( edit ) );
	}
...
namespace cfcore {
	...
	std::string CFGenKbGelBuiltinObj::expand( MssCFGenContext* genContext ) {
