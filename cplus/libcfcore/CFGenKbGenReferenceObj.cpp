// Description: C++18 base object instance implementation for CFGenKb GenReference.
...
namespace cfcore {
	...
	const std::string CFGenKbGenReferenceObj::CLASS_NAME( "CFGenKbGenReferenceObj" );
	const classcode_t CFGenKbGenReferenceObj::CLASS_CODE = 0xa922L;


	CFGenKbGenReferenceObj::CFGenKbGenReferenceObj()
	: ICFGenKbGenReferenceObj(),
	  CFGenKbGenItemObj()
	{
	}

	CFGenKbGenReferenceObj::CFGenKbGenReferenceObj( cfcore::ICFGenKbSchemaObj* argSchema )
	: ICFGenKbGenReferenceObj(),
	...
	cfcore::CFGenKbGenItemBuff* CFGenKbGenReferenceObj::getBuff() {
		if( buff == NULL ) {
			if( isNew ) {
				buff = new cfcore::CFGenKbGenReferenceBuff();
				copyBuffToPKey();
			}
			else {
				// Read the data buff via the backing store
				buff = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGenReference()->readDerivedByItemIdIdx( dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getAuthorization(),
	...
	void CFGenKbGenReferenceObj::setBuff( cfcore::CFGenKbGenItemBuff* value ) {
	...
	const std::string& CFGenKbGenReferenceObj::getClassName() const {
		return( CLASS_NAME );
	}

	const classcode_t CFGenKbGenReferenceObj::getClassCode() const {
		return( CLASS_CODE );
	}

	bool CFGenKbGenReferenceObj::implementsClassCode( const classcode_t value ) const {
	...
	std::string CFGenKbGenReferenceObj::toString() {
		static const std::string S_Space( " " );
		static const std::string S_Preamble( "<CFGenKbGenReferenceObj" );
	...
	std::string CFGenKbGenReferenceObj::getObjName() {
	...
	const std::string CFGenKbGenReferenceObj::getGenDefName() {
		return( cfcore::CFGenKbGenReferenceBuff::GENDEFNAME );
	}

	cflib::ICFLibAnyObj* CFGenKbGenReferenceObj::getObjQualifier( const classcode_t* qualifyingClass ) {
	...
	cflib::ICFLibAnyObj* CFGenKbGenReferenceObj::getNamedObject( const classcode_t* qualifyingClass, const std::string& objName ) {
	...
	cflib::ICFLibAnyObj* CFGenKbGenReferenceObj::getNamedObject( const std::string& objName ) {
		std::string nextName;
	...
	std::string CFGenKbGenReferenceObj::getObjQualifiedName() {
	...
	std::string CFGenKbGenReferenceObj::getObjFullName() {
		const static std::string S_Dot( "." );
	...
	cfcore::ICFGenKbGenItemObj* CFGenKbGenReferenceObj::realize() {
		cfcore::ICFGenKbGenReferenceObj* retobj = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGenReferenceTableObj()->realizeGenReference(
			(cfcore::ICFGenKbGenReferenceObj*)this );
		return( dynamic_cast<cfcore::ICFGenKbGenItemObj*>( retobj ) );
	}

	cfcore::ICFGenKbGenItemObj* CFGenKbGenReferenceObj::read( bool forceRead ) {
		cfcore::ICFGenKbGenReferenceObj* retobj = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGenReferenceTableObj()->readGenReferenceByItemIdIdx( getGenReferenceBuff()->getRequiredTenantId(),
		getGenReferenceBuff()->getRequiredCartridgeId(),
		getGenReferenceBuff()->getRequiredItemId(), forceRead );
		return( dynamic_cast<cfcore::ICFGenKbGenItemObj*>( retobj ) );
	}

	cfcore::ICFGenKbGenItemEditObj* CFGenKbGenReferenceObj::beginEdit() {
		...
		if( ! getIsNew() ) {
			cfcore::ICFGenKbGenReferenceObj* lockobj = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGenReferenceTableObj()->lockGenReference( getPKey() );
			lockobj->realize();
		}
		edit = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGenReferenceTableObj()->newEditInstance( this );
		return( dynamic_cast<cfcore::ICFGenKbGenItemEditObj*>( edit ) );
	}

	cfcore::ICFGenKbGenReferenceEditObj* CFGenKbGenReferenceObj::getGenReferenceEdit() {
		return( dynamic_cast<cfcore::ICFGenKbGenReferenceEditObj*>( edit ) );
	}
