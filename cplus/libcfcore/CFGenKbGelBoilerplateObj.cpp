// Description: C++18 base object instance implementation for CFGenKb GelBoilerplate.
...
namespace cfcore {
	...
	const std::string CFGenKbGelBoilerplateObj::CLASS_NAME( "CFGenKbGelBoilerplateObj" );
	const classcode_t CFGenKbGelBoilerplateObj::CLASS_CODE = 0xa929L;


	CFGenKbGelBoilerplateObj::CFGenKbGelBoilerplateObj()
	: ICFGenKbGelBoilerplateObj(),
	  CFGenKbGelInstructionObj()
	{
	}

	CFGenKbGelBoilerplateObj::CFGenKbGelBoilerplateObj( cfcore::ICFGenKbSchemaObj* argSchema )
	: ICFGenKbGelBoilerplateObj(),
	...
	cfcore::CFGenKbGelInstructionBuff* CFGenKbGelBoilerplateObj::getBuff() {
		if( buff == NULL ) {
			if( isNew ) {
				buff = new cfcore::CFGenKbGelBoilerplateBuff();
				copyBuffToPKey();
			}
			else {
				// Read the data buff via the backing store
				buff = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelBoilerplate()->readDerivedByPIdx( dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getAuthorization(),
	...
	void CFGenKbGelBoilerplateObj::setBuff( cfcore::CFGenKbGelInstructionBuff* value ) {
	...
	const std::string& CFGenKbGelBoilerplateObj::getClassName() const {
		return( CLASS_NAME );
	}

	const classcode_t CFGenKbGelBoilerplateObj::getClassCode() const {
		return( CLASS_CODE );
	}

	bool CFGenKbGelBoilerplateObj::implementsClassCode( const classcode_t value ) const {
	...
	std::string CFGenKbGelBoilerplateObj::toString() {
		static const std::string S_Space( " " );
		static const std::string S_Preamble( "<CFGenKbGelBoilerplateObj" );
	...
	std::string CFGenKbGelBoilerplateObj::getObjName() {
	...
	const std::string CFGenKbGelBoilerplateObj::getGenDefName() {
		return( cfcore::CFGenKbGelBoilerplateBuff::GENDEFNAME );
	}

	cflib::ICFLibAnyObj* CFGenKbGelBoilerplateObj::getObjQualifier( const classcode_t* qualifyingClass ) {
	...
	cflib::ICFLibAnyObj* CFGenKbGelBoilerplateObj::getNamedObject( const classcode_t* qualifyingClass, const std::string& objName ) {
	...
	cflib::ICFLibAnyObj* CFGenKbGelBoilerplateObj::getNamedObject( const std::string& objName ) {
		std::string nextName;
	...
	std::string CFGenKbGelBoilerplateObj::getObjQualifiedName() {
	...
	std::string CFGenKbGelBoilerplateObj::getObjFullName() {
		const static std::string S_Dot( "." );
	...
	cfcore::ICFGenKbGelInstructionObj* CFGenKbGelBoilerplateObj::realize() {
		cfcore::ICFGenKbGelBoilerplateObj* retobj = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGelBoilerplateTableObj()->realizeGelBoilerplate(
			(cfcore::ICFGenKbGelBoilerplateObj*)this );
		return( dynamic_cast<cfcore::ICFGenKbGelInstructionObj*>( retobj ) );
	}

	cfcore::ICFGenKbGelInstructionObj* CFGenKbGelBoilerplateObj::read( bool forceRead ) {
		cfcore::ICFGenKbGelBoilerplateObj* retobj = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGelBoilerplateTableObj()->readGelBoilerplateByPIdx( getGelBoilerplateBuff()->getRequiredTenantId(),
		getGelBoilerplateBuff()->getRequiredGelCacheId(),
		getGelBoilerplateBuff()->getRequiredGelInstId(), forceRead );
		return( dynamic_cast<cfcore::ICFGenKbGelInstructionObj*>( retobj ) );
	}

	cfcore::ICFGenKbGelInstructionEditObj* CFGenKbGelBoilerplateObj::beginEdit() {
		...
		if( ! getIsNew() ) {
			cfcore::ICFGenKbGelBoilerplateObj* lockobj = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGelBoilerplateTableObj()->lockGelBoilerplate( getPKey() );
			lockobj->realize();
		}
		edit = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGelBoilerplateTableObj()->newEditInstance( this );
		return( dynamic_cast<cfcore::ICFGenKbGelInstructionEditObj*>( edit ) );
	}

	cfcore::ICFGenKbGelBoilerplateEditObj* CFGenKbGelBoilerplateObj::getGelBoilerplateEdit() {
		return( dynamic_cast<cfcore::ICFGenKbGelBoilerplateEditObj*>( edit ) );
	}
...
namespace cfcore {
	...
	std::string CFGenKbGelBoilerplateObj::expand( MssCFGenContext* genContext ) {
