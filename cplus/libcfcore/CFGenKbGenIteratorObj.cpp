// Description: C++18 base object instance implementation for CFGenKb GenIterator.
...
namespace cfcore {
	...
	const std::string CFGenKbGenIteratorObj::CLASS_NAME( "CFGenKbGenIteratorObj" );
	const classcode_t CFGenKbGenIteratorObj::CLASS_CODE = 0xa921L;


	CFGenKbGenIteratorObj::CFGenKbGenIteratorObj()
	: ICFGenKbGenIteratorObj(),
	  CFGenKbGenItemObj()
	{
	}

	CFGenKbGenIteratorObj::CFGenKbGenIteratorObj( cfcore::ICFGenKbSchemaObj* argSchema )
	: ICFGenKbGenIteratorObj(),
	...
	cfcore::CFGenKbGenItemBuff* CFGenKbGenIteratorObj::getBuff() {
		if( buff == NULL ) {
			if( isNew ) {
				buff = new cfcore::CFGenKbGenIteratorBuff();
				copyBuffToPKey();
			}
			else {
				// Read the data buff via the backing store
				buff = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGenIterator()->readDerivedByItemIdIdx( dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getAuthorization(),
	...
	void CFGenKbGenIteratorObj::setBuff( cfcore::CFGenKbGenItemBuff* value ) {
	...
	const std::string& CFGenKbGenIteratorObj::getClassName() const {
		return( CLASS_NAME );
	}

	const classcode_t CFGenKbGenIteratorObj::getClassCode() const {
		return( CLASS_CODE );
	}

	bool CFGenKbGenIteratorObj::implementsClassCode( const classcode_t value ) const {
	...
	std::string CFGenKbGenIteratorObj::toString() {
		static const std::string S_Space( " " );
		static const std::string S_Preamble( "<CFGenKbGenIteratorObj" );
	...
	std::string CFGenKbGenIteratorObj::getObjName() {
	...
	const std::string CFGenKbGenIteratorObj::getGenDefName() {
		return( cfcore::CFGenKbGenIteratorBuff::GENDEFNAME );
	}

	cflib::ICFLibAnyObj* CFGenKbGenIteratorObj::getObjQualifier( const classcode_t* qualifyingClass ) {
	...
	cflib::ICFLibAnyObj* CFGenKbGenIteratorObj::getNamedObject( const classcode_t* qualifyingClass, const std::string& objName ) {
	...
	cflib::ICFLibAnyObj* CFGenKbGenIteratorObj::getNamedObject( const std::string& objName ) {
		std::string nextName;
	...
	std::string CFGenKbGenIteratorObj::getObjQualifiedName() {
	...
	std::string CFGenKbGenIteratorObj::getObjFullName() {
		const static std::string S_Dot( "." );
	...
	cfcore::ICFGenKbGenItemObj* CFGenKbGenIteratorObj::realize() {
		cfcore::ICFGenKbGenIteratorObj* retobj = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGenIteratorTableObj()->realizeGenIterator(
			(cfcore::ICFGenKbGenIteratorObj*)this );
		return( dynamic_cast<cfcore::ICFGenKbGenItemObj*>( retobj ) );
	}

	cfcore::ICFGenKbGenItemObj* CFGenKbGenIteratorObj::read( bool forceRead ) {
		cfcore::ICFGenKbGenIteratorObj* retobj = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGenIteratorTableObj()->readGenIteratorByItemIdIdx( getGenIteratorBuff()->getRequiredTenantId(),
		getGenIteratorBuff()->getRequiredCartridgeId(),
		getGenIteratorBuff()->getRequiredItemId(), forceRead );
		return( dynamic_cast<cfcore::ICFGenKbGenItemObj*>( retobj ) );
	}

	cfcore::ICFGenKbGenItemEditObj* CFGenKbGenIteratorObj::beginEdit() {
		...
		if( ! getIsNew() ) {
			cfcore::ICFGenKbGenIteratorObj* lockobj = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGenIteratorTableObj()->lockGenIterator( getPKey() );
			lockobj->realize();
		}
		edit = dynamic_cast<ICFGenKbSchemaObj*>( getSchema() )->getGenIteratorTableObj()->newEditInstance( this );
		return( dynamic_cast<cfcore::ICFGenKbGenItemEditObj*>( edit ) );
	}

	cfcore::ICFGenKbGenIteratorEditObj* CFGenKbGenIteratorObj::getGenIteratorEdit() {
		return( dynamic_cast<cfcore::ICFGenKbGenIteratorEditObj*>( edit ) );
	}
