...
namespace cfcore {
	...
	const std::string CFGenKbGelCounterTableObj::CLASS_NAME( "CFGenKbGelCounterTableObj" );
	const std::string CFGenKbGelCounterTableObj::TABLE_NAME( "GelCounter" );
	const std::string CFGenKbGelCounterTableObj::TABLE_DBNAME( "gelcounter" );

	CFGenKbGelCounterTableObj::CFGenKbGelCounterTableObj() {
		schema = NULL;
		members = new std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>();
		allGelCounter = NULL;
		indexByTenantIdx = new std::map< cfcore::CFGenKbGelInstructionByTenantIdxKey,
			std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>*>();
	...
	CFGenKbGelCounterTableObj::CFGenKbGelCounterTableObj( cfcore::ICFGenKbSchemaObj* argSchema ) {
	...
	cfcore::ICFGenKbSchemaObj* CFGenKbGelCounterTableObj::getSchema() {
		return( schema );
	}

	void CFGenKbGelCounterTableObj::setSchema( cfcore::ICFGenKbSchemaObj* value ) {
		schema = dynamic_cast<cfcore::ICFGenKbSchemaObj*>( value );
	}

	const std::string CFGenKbGelCounterTableObj::getTableName() {
		return( TABLE_NAME );
	}

	const std::string CFGenKbGelCounterTableObj::getTableDbName() {
		return( TABLE_DBNAME );
	}
	const classcode_t* CFGenKbGelCounterTableObj::getObjQualifyingClassCode() {
		return( NULL );
	}


	void CFGenKbGelCounterTableObj::minimizeMemory() {
		if( allGelCounter != NULL ) {
			allGelCounter->clear();
			delete allGelCounter;
			allGelCounter = NULL;
		}
		if( indexByTenantIdx != NULL ) {
			std::map< cfcore::CFGenKbGelInstructionByTenantIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* >::iterator iterByTenantIdx = indexByTenantIdx->begin();
			std::map< cfcore::CFGenKbGelInstructionByTenantIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* >::iterator endByTenantIdx = indexByTenantIdx->end();
			std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* curByTenantIdx = NULL;
		...
		if( indexByGelCacheIdx != NULL ) {
			std::map< cfcore::CFGenKbGelInstructionByGelCacheIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* >::iterator iterByGelCacheIdx = indexByGelCacheIdx->begin();
			std::map< cfcore::CFGenKbGelInstructionByGelCacheIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* >::iterator endByGelCacheIdx = indexByGelCacheIdx->end();
			std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* curByGelCacheIdx = NULL;
		...
		if( indexByChainIdx != NULL ) {
			std::map< cfcore::CFGenKbGelInstructionByChainIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* >::iterator iterByChainIdx = indexByChainIdx->begin();
			std::map< cfcore::CFGenKbGelInstructionByChainIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* >::iterator endByChainIdx = indexByChainIdx->end();
			std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* curByChainIdx = NULL;
	...
	cfcore::ICFGenKbGelCounterObj* CFGenKbGelCounterTableObj::newInstance() {
		cfcore::ICFGenKbGelCounterObj* inst = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( new CFGenKbGelCounterObj( schema ) );
		return( inst );
	}

	cfcore::ICFGenKbGelCounterEditObj* CFGenKbGelCounterTableObj::newEditInstance( cfcore::ICFGenKbGelCounterObj* orig ) {
		cfcore::ICFGenKbGelCounterEditObj* edit = dynamic_cast<cfcore::ICFGenKbGelCounterEditObj*>( new CFGenKbGelCounterEditObj( orig ));
		return( edit );
	}

	cfcore::ICFGenKbGelCounterObj* CFGenKbGelCounterTableObj::realizeGelCounter( cfcore::ICFGenKbGelCounterObj* Obj ) {
		static const std::string S_ProcName( "realizeGelCounter" );
		...
		cfcore::ICFGenKbGelCounterObj* obj = Obj;
		cfcore::ICFGenKbGelCounterObj* existingObj = NULL;
		cfcore::CFGenKbGelInstructionPKey* pkey = obj->getPKey();
		cfcore::ICFGenKbGelCounterObj* keepObj = NULL;
		...
		if( searchMembers != members->end() ) {
			...
			if( indexByTenantIdx != NULL ) {
				...
				if( searchTenantIdx != indexByTenantIdx->end() ) {
					std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* mapTenantIdx = searchTenantIdx->second;
			...
			if( indexByGelCacheIdx != NULL ) {
				...
				if( searchGelCacheIdx != indexByGelCacheIdx->end() ) {
					std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* mapGelCacheIdx = searchGelCacheIdx->second;
			...
			if( indexByChainIdx != NULL ) {
				...
				if( searchChainIdx != indexByChainIdx->end() ) {
					std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* mapChainIdx = searchChainIdx->second;
					if( mapChainIdx != NULL ) {
			...
			keepObj = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( dynamic_cast<cfcore::CFGenKbGelInstructionTableObj*>( schema->getGelInstructionTableObj() )->realizeGelInstruction( Obj ) );
			if( keepObj == NULL ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
					S_ProcName,
					0,
			...
			if( indexByTenantIdx != NULL ) {
				...
				cfcore::ICFGenKbGelCounterObj* aTenantIdxObj =
					dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( keepObj );
				...
				if( searchIndexByTenantIdx != indexByTenantIdx->end() ) {
					std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* mapTenantIdx = searchIndexByTenantIdx->second;
					if( mapTenantIdx != NULL ) {
						mapTenantIdx->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj* >::value_type( *(keepObj->getPKey()), aTenantIdxObj ) );
					}
				}
			}
			...
			if( indexByGelCacheIdx != NULL ) {
				...
				cfcore::ICFGenKbGelCounterObj* aGelCacheIdxObj =
					dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( keepObj );
				...
				if( searchIndexByGelCacheIdx != indexByGelCacheIdx->end() ) {
					std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* mapGelCacheIdx = searchIndexByGelCacheIdx->second;
					if( mapGelCacheIdx != NULL ) {
						mapGelCacheIdx->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj* >::value_type( *(keepObj->getPKey()), aGelCacheIdxObj ) );
					}
				}
			}
			...
			if( indexByChainIdx != NULL ) {
				...
				cfcore::ICFGenKbGelCounterObj* aChainIdxObj =
					dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( keepObj );
				...
				if( searchIndexByChainIdx != indexByChainIdx->end() ) {
					std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* mapChainIdx = searchIndexByChainIdx->second;
					if( mapChainIdx != NULL ) {
						mapChainIdx->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj* >::value_type( *(keepObj->getPKey()), aChainIdxObj ) );
					}
				}
			}

			if( allGelCounter != NULL ) {
				allGelCounter->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj* >::value_type( *(keepObj->getPKey()), keepObj ) );
			}
		}
		else {
			keepObj = obj;
			...
			members->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj* >::value_type( *(keepObj->getPKey()), keepObj ) );
	...
	void CFGenKbGelCounterTableObj::deepDisposeByPIdx( const int64_t TenantId,
			const int64_t GelCacheId,
			const int64_t GelInstId ) {
		static const std::string S_ProcName( "deepDisposeByPIdx" );
		std::vector<cfcore::ICFGenKbGelCounterObj*> list;
		cfcore::ICFGenKbGelCounterObj* existingObj = readCachedGelCounterByPIdx( TenantId,
				GelCacheId,
				GelInstId );
		if( existingObj != NULL ) {
			list.push_back( existingObj );
		}
		cfcore::ICFGenKbGelCounterObj* cur = NULL;
		...
		while( listIter != listEnd ) {
			...
			if( cur != NULL ) {
				...
				if( classCode == cfcore::CFGenKbGelCounterBuff::CLASS_CODE ) {
	...
	void CFGenKbGelCounterTableObj::deepDisposeByTenantIdx( const int64_t TenantId ) {
		static const std::string S_ProcName( "deepDisposeByTenantIdx" );
		std::vector<cfcore::ICFGenKbGelCounterObj*> list;
		std::vector<cfcore::ICFGenKbGelCounterObj*> matchesFound = readCachedGelCounterByTenantIdx( TenantId );
	...
	void CFGenKbGelCounterTableObj::deepDisposeByGelCacheIdx( const int64_t TenantId,
			const int64_t GelCacheId ) {
		static const std::string S_ProcName( "deepDisposeByGelCacheIdx" );
		std::vector<cfcore::ICFGenKbGelCounterObj*> list;
		std::vector<cfcore::ICFGenKbGelCounterObj*> matchesFound = readCachedGelCounterByGelCacheIdx( TenantId,
				GelCacheId );
		auto iterMatches = matchesFound.begin();
		auto endMatches = matchesFound.end();
		while( iterMatches != endMatches ) {
	...
	void CFGenKbGelCounterTableObj::deepDisposeByChainIdx( const int64_t* ChainInstTenantId,
			const int64_t* ChainInstGelCacheId,
			const int64_t* ChainInstGelInstId ) {
		static const std::string S_ProcName( "deepDisposeByChainIdx" );
		std::vector<cfcore::ICFGenKbGelCounterObj*> list;
		std::vector<cfcore::ICFGenKbGelCounterObj*> matchesFound = readCachedGelCounterByChainIdx( ChainInstTenantId,
				ChainInstGelCacheId,
				ChainInstGelInstId );
		auto iterMatches = matchesFound.begin();
		auto endMatches = matchesFound.end();
	...
	void CFGenKbGelCounterTableObj::reallyDeepDisposeGelCounter( cfcore::ICFGenKbGelCounterObj* Obj ) {
		static const std::string S_ProcName( "reallyDeepDisposeGelCounter" );
	...
	cfcore::ICFGenKbGelCounterObj* CFGenKbGelCounterTableObj::createGelCounter( cfcore::ICFGenKbGelCounterEditObj* Obj ) {
		static const std::string S_ProcName( "createGelCounter" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Cloneable( "cloneable" );
		static const std::string S_ClonedBuff( "clonedbuff" );
		CFLIB_EXCEPTION_DECLINFO
		cfcore::ICFGenKbGelCounterObj* obj = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( Obj->getOrig() );
		try {
			cfcore::CFGenKbGelCounterBuff* buff = dynamic_cast<cfcore::CFGenKbGelCounterBuff*>( Obj->getBuff()->clone() );
			// C++18 version of create returns a new buffer instance and takes over ownership of the passed-in buffer
			// MSS TODO WORKING The xmsg client will need to return the buffer instance created by processing
			// the response message, while xmsg rqst will have to delete the backing store instance
			// it receives after preparing the reply message so that memory doesn't leak on every request.
			cflib::ICFLibCloneableObj* cloneable = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelCounter()->createGelCounter(
			...
			Obj->endEdit();
			obj->setBuff( dynamic_cast<cfcore::CFGenKbGelCounterBuff*>( cloneable ) );
			obj = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( obj->realize() );
			if( obj == NULL ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
					S_ProcName,
					0,
	...
	cfcore::ICFGenKbGelCounterObj* CFGenKbGelCounterTableObj::readGelCounter( cfcore::CFGenKbGelInstructionPKey* pkey, bool forceRead ) {
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		static const std::string S_ProcName( "readGelCounter" );
		cfcore::ICFGenKbGelCounterObj* obj = NULL;
		cfcore::ICFGenKbGelCounterObj* realized = NULL;
		if( ! forceRead ) {
			auto searchMembers = members->find( *pkey );
			if( searchMembers != members->end() ) {
		...
		if( forceRead || ( obj == NULL ) ) {
			cfcore::CFGenKbGelCounterBuff* readBuff = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelCounter()->readDerivedByPIdx( schema->getAuthorization(),
			pkey->getRequiredTenantId(),
			pkey->getRequiredGelCacheId(),
			pkey->getRequiredGelInstId() );
			if( readBuff != NULL ) {
			obj = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( dynamic_cast<cfcore::CFGenKbGelInstructionTableObj*>( schema->getGelInstructionTableObj() )->constructByClassCode( readBuff->getClassCode() ) );
				obj->setBuff( readBuff );
				realized = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( obj->realize() );
				if( realized == NULL ) {
					throw cflib::CFLibNullArgumentException( CLASS_NAME,
						S_ProcName,
						0,
	...
	cfcore::ICFGenKbGelCounterObj* CFGenKbGelCounterTableObj::lockGelCounter( cfcore::CFGenKbGelInstructionPKey* pkey ) {
		static const std::string S_ProcName( "lockGelCounter" );
		cfcore::ICFGenKbGelCounterObj* locked = NULL;
		cfcore::CFGenKbGelCounterBuff* lockBuff = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelCounter()->lockDerived( schema->getAuthorization(), pkey );
		if( lockBuff != NULL ) {
			locked = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( dynamic_cast<cfcore::CFGenKbGelInstructionTableObj*>( schema->getGelInstructionTableObj() )->constructByClassCode( lockBuff->getClassCode() ) );
			locked->setBuff( lockBuff );
			locked = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( locked->realize() );
		}
		else {
			return( NULL );
		}
	...
	std::vector<cfcore::ICFGenKbGelCounterObj*> CFGenKbGelCounterTableObj::readAllGelCounter( bool forceRead ) {
		static const std::string S_ProcName( "readAllGelCounter" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Realized( "realized" );
		CFLIB_EXCEPTION_DECLINFO
		cfcore::ICFGenKbGelCounterObj* realized = NULL;
		if( forceRead || ( allGelCounter == NULL ) ) {
			std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* map = new std::map<cfcore::CFGenKbGelInstructionPKey,cfcore::ICFGenKbGelCounterObj*>();
			allGelCounter = map;
			std::TCFLibOwningVector<cfcore::CFGenKbGelCounterBuff*> buffList = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelCounter()->readAllDerived( schema->getAuthorization() );
			cfcore::CFGenKbGelCounterBuff* buff = NULL;
			cfcore::ICFGenKbGelCounterObj* obj = NULL;
			try {
				for( size_t idx = 0; idx < buffList.size(); idx ++ ) {
					buff = buffList[ idx ];
					buffList[ idx ] = NULL;
				obj = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( dynamic_cast<cfcore::CFGenKbGelInstructionTableObj*>( schema->getGelInstructionTableObj() )->constructByClassCode( buff->getClassCode() ) );
					obj->setBuff( buff );
					realized = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( obj->realize() );
					if( realized == NULL ) {
						throw cflib::CFLibNullArgumentException( CLASS_NAME,
					...
					allGelCounter->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj* >::value_type( *(realized->getPKey()), realized ) );
					// No need to delete obj -- realize() auto-destructs the instance it decided to discard
					obj = NULL;
				}
			}
		...
		size_t len = allGelCounter->size();
		std::vector<cfcore::ICFGenKbGelCounterObj*> arr;
		auto valIter = allGelCounter->begin();
		size_t idx = 0;
		while( valIter != allGelCounter->end() ) {
	...
	cfcore::ICFGenKbGelCounterObj* CFGenKbGelCounterTableObj::readGelCounterByPIdx( const int64_t TenantId,
			const int64_t GelCacheId,
			const int64_t GelInstId, bool forceRead )
	{
		static const std::string S_ProcName( "readGelCounterByPIdx" );
		static const std::string S_Realized( "realized" );
		cfcore::CFGenKbGelInstructionPKey pkey;
		pkey.setRequiredTenantId( TenantId );
		pkey.setRequiredGelCacheId( GelCacheId );
		pkey.setRequiredGelInstId( GelInstId );
		cfcore::ICFGenKbGelCounterObj* obj = readGelCounter( &pkey, forceRead );
		return( obj );
	}

	std::vector<cfcore::ICFGenKbGelCounterObj*> CFGenKbGelCounterTableObj::readGelCounterByTenantIdx( const int64_t TenantId,
		bool forceRead )
	{
		static const std::string S_ProcName( "readGelCounterByTenantIdx" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		cfcore::CFGenKbGelInstructionByTenantIdxKey key;
		key.setRequiredTenantId( TenantId );
		std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* dict;
		std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>* oldDict;
		if( indexByTenantIdx == NULL ) {
			indexByTenantIdx = new std::map< cfcore::CFGenKbGelInstructionByTenantIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>*>();
		}
		...
		if( ( oldDict != NULL ) && ( ! forceRead ) ) {
			...
			dict = new std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj*>();
			...
			for( size_t idx = 0; idx < buffList.size(); idx ++ ) {
				buff = buffList[ idx ];
				buffList[ idx ] = NULL;
				obj = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>(
				...
				obj->setBuff( buff );
				cfcore::ICFGenKbGelCounterObj* realized = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( obj->realize() );
				if( realized == NULL ) {
					throw cflib::CFLibNullArgumentException( CLASS_NAME,
						S_ProcName,
						0,
				...
				dict->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj* >::value_type( *(realized->getPKey()), realized ) );
				// No need to delete obj -- realize() auto-destructs the instance it decided to discard
				obj = NULL;
			}
			if( oldDict != NULL ) {
			...
			indexByTenantIdx->insert( std::map< cfcore::CFGenKbGelInstructionByTenantIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelCounterObj* >* >::value_type( key, dict ) );
		}
	...
	std::vector<cfcore::ICFGenKbGelCounterObj*> CFGenKbGelCounterTableObj::readGelCounterByGelCacheIdx( const int64_t TenantId,
			const int64_t GelCacheId,
		bool forceRead )
	{
		static const std::string S_ProcName( "readGelCounterByGelCacheIdx" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
	...
	std::vector<cfcore::ICFGenKbGelCounterObj*> CFGenKbGelCounterTableObj::readGelCounterByChainIdx( const int64_t* ChainInstTenantId,
			const int64_t* ChainInstGelCacheId,
			const int64_t* ChainInstGelInstId,
		bool forceRead )
	{
		static const std::string S_ProcName( "readGelCounterByChainIdx" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
	...
	cfcore::ICFGenKbGelCounterObj* CFGenKbGelCounterTableObj::readCachedGelCounter( cfcore::CFGenKbGelInstructionPKey* pkey ) {
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		static const std::string S_ProcName( "readGelCounter" );
		cfcore::ICFGenKbGelCounterObj* obj = NULL;
	...
	cfcore::ICFGenKbGelCounterObj* CFGenKbGelCounterTableObj::readCachedGelCounterByPIdx( const int64_t TenantId,
			const int64_t GelCacheId,
			const int64_t GelInstId )
	{
		static const std::string S_ProcName( "readCachedGelCounterByPIdx" );
		static const std::string S_Realized( "realized" );
		cfcore::CFGenKbGelInstructionPKey pkey;
		pkey.setRequiredTenantId( TenantId );
		pkey.setRequiredGelCacheId( GelCacheId );
		pkey.setRequiredGelInstId( GelInstId );
		cfcore::ICFGenKbGelCounterObj* obj = readCachedGelCounter( &pkey );
		return( obj );
	}

	std::vector<cfcore::ICFGenKbGelCounterObj*> CFGenKbGelCounterTableObj::readCachedGelCounterByTenantIdx( const int64_t TenantId )
	{
		static const std::string S_ProcName( "readCachedGelCounterByTenantIdx" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		std::vector<cfcore::ICFGenKbGelCounterObj*> arr;
		...
		if( searchIndexByTenantIdx != indexByTenantIdx->end() ) {
			...
			cfcore::ICFGenKbGelCounterObj* obj;
	...
	std::vector<cfcore::ICFGenKbGelCounterObj*> CFGenKbGelCounterTableObj::readCachedGelCounterByGelCacheIdx( const int64_t TenantId,
			const int64_t GelCacheId )
	{
		static const std::string S_ProcName( "readCachedGelCounterByGelCacheIdx" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		std::vector<cfcore::ICFGenKbGelCounterObj*> arr;
	...
	std::vector<cfcore::ICFGenKbGelCounterObj*> CFGenKbGelCounterTableObj::readCachedGelCounterByChainIdx( const int64_t* ChainInstTenantId,
			const int64_t* ChainInstGelCacheId,
			const int64_t* ChainInstGelInstId )
	{
		static const std::string S_ProcName( "readCachedGelCounterByChainIdx" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		std::vector<cfcore::ICFGenKbGelCounterObj*> arr;
	...
	cfcore::ICFGenKbGelCounterObj* CFGenKbGelCounterTableObj::updateGelCounter( cfcore::ICFGenKbGelCounterEditObj* Obj ) {
		static const std::string S_ProcName( "updateGelCounter" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Updated( "updated" );
		CFLIB_EXCEPTION_DECLINFO
		cfcore::ICFGenKbGelCounterObj* obj = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( Obj->getOrig() );
		try {
			cfcore::CFGenKbGelCounterBuff* updated = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelCounter()->updateGelCounter( schema->getAuthorization(),
				dynamic_cast<cfcore::CFGenKbGelCounterBuff*>( Obj->getGelCounterBuff()->clone() ) );
			if( updated == NULL ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
					S_ProcName,
					0,
			...
			obj = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( dynamic_cast<cfcore::CFGenKbGelCounterTableObj*>( schema->getGelCounterTableObj() )->newInstance() );
			obj->setBuff( updated );
			obj = dynamic_cast<cfcore::ICFGenKbGelCounterObj*>( obj->realize() );
			if( obj == NULL ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
	...
	void CFGenKbGelCounterTableObj::deleteGelCounter( cfcore::ICFGenKbGelCounterEditObj* Obj ) {
		cfcore::ICFGenKbGelCounterObj* obj = Obj;
		dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelCounter()->deleteGelCounter( schema->getAuthorization(),
	...
	void CFGenKbGelCounterTableObj::deleteGelCounterByPIdx( const int64_t TenantId,
			const int64_t GelCacheId,
			const int64_t GelInstId )
	{
		cfcore::CFGenKbGelInstructionPKey pkey;
		...
		pkey.setRequiredGelCacheId( GelCacheId );
		pkey.setRequiredGelInstId( GelInstId );
		cfcore::ICFGenKbGelCounterObj* obj = readGelCounter( &pkey, true );
		if( obj != NULL ) {
			cfcore::ICFGenKbGelCounterEditObj* editObj = dynamic_cast<cfcore::ICFGenKbGelCounterEditObj*>( obj->getEdit() );
			if( editObj == NULL ) {
				editObj = dynamic_cast<cfcore::ICFGenKbGelCounterEditObj*>( obj->beginEdit() );
			}
			if( editObj != NULL ) {
	...
	void CFGenKbGelCounterTableObj::deleteGelCounterByTenantIdx( const int64_t TenantId )
	{
		dynamic_cast<cfcore::ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelCounter()->deleteGelCounterByTenantIdx( schema->getAuthorization(),
				TenantId );
		deepDisposeByTenantIdx( TenantId );
	}

	void CFGenKbGelCounterTableObj::deleteGelCounterByGelCacheIdx( const int64_t TenantId,
			const int64_t GelCacheId )
	{
		dynamic_cast<cfcore::ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelCounter()->deleteGelCounterByGelCacheIdx( schema->getAuthorization(),
				TenantId,
				GelCacheId );
	...
	void CFGenKbGelCounterTableObj::deleteGelCounterByChainIdx( const int64_t* ChainInstTenantId,
			const int64_t* ChainInstGelCacheId,
			const int64_t* ChainInstGelInstId )
	{
		dynamic_cast<cfcore::ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelCounter()->deleteGelCounterByChainIdx( schema->getAuthorization(),
				ChainInstTenantId,
				ChainInstGelCacheId,
				ChainInstGelInstId );
	...
	void CFGenKbGelCounterTableObj::reallyDetachFromIndexesGelCounter( cfcore::ICFGenKbGelCounterObj* Obj ) {
		static const std::string S_ProcName( "reallyDetachFromIndexesGelCounter" );
		static const std::string S_Obj( "Obj" );
		static const std::string S_ExistingObj( "ExistingObj" );
		if( Obj == NULL ) {
			throw cflib::CFLibNullArgumentException( CLASS_NAME, S_ProcName, 1, S_Obj );
		...
		auto searchMembers = members->find( *pkey );
		if( searchMembers != members->end() ) {
			cfcore::ICFGenKbGelCounterObj* existingObj = searchMembers->second;
			if( existingObj == NULL ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
					S_ProcName,
					0,
