...
namespace cfcore {
	...
	const std::string CFGenKbGelPopTableObj::CLASS_NAME( "CFGenKbGelPopTableObj" );
	const std::string CFGenKbGelPopTableObj::TABLE_NAME( "GelPop" );
	const std::string CFGenKbGelPopTableObj::TABLE_DBNAME( "gelpop" );

	CFGenKbGelPopTableObj::CFGenKbGelPopTableObj() {
		schema = NULL;
		members = new std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>();
		allGelPop = NULL;
		indexByTenantIdx = new std::map< cfcore::CFGenKbGelInstructionByTenantIdxKey,
			std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>*>();
		...
		indexByChainIdx = new std::map< cfcore::CFGenKbGelInstructionByChainIdxKey,
			std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>*>();
		indexByRemainderIdx = new std::map< cfcore::CFGenKbGelPopByRemainderIdxKey,
			std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>*>();
	}

	CFGenKbGelPopTableObj::CFGenKbGelPopTableObj( cfcore::ICFGenKbSchemaObj* argSchema ) {
	...
	cfcore::ICFGenKbSchemaObj* CFGenKbGelPopTableObj::getSchema() {
		return( schema );
	}

	void CFGenKbGelPopTableObj::setSchema( cfcore::ICFGenKbSchemaObj* value ) {
		schema = dynamic_cast<cfcore::ICFGenKbSchemaObj*>( value );
	}

	const std::string CFGenKbGelPopTableObj::getTableName() {
		return( TABLE_NAME );
	}

	const std::string CFGenKbGelPopTableObj::getTableDbName() {
		return( TABLE_DBNAME );
	}
	const classcode_t* CFGenKbGelPopTableObj::getObjQualifyingClassCode() {
		return( NULL );
	}


	void CFGenKbGelPopTableObj::minimizeMemory() {
		if( allGelPop != NULL ) {
			allGelPop->clear();
			delete allGelPop;
			allGelPop = NULL;
		...
		if( indexByTenantIdx != NULL ) {
			std::map< cfcore::CFGenKbGelInstructionByTenantIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* >::iterator iterByTenantIdx = indexByTenantIdx->begin();
			std::map< cfcore::CFGenKbGelInstructionByTenantIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* >::iterator endByTenantIdx = indexByTenantIdx->end();
			std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* curByTenantIdx = NULL;
		...
		if( indexByGelCacheIdx != NULL ) {
			std::map< cfcore::CFGenKbGelInstructionByGelCacheIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* >::iterator iterByGelCacheIdx = indexByGelCacheIdx->begin();
			std::map< cfcore::CFGenKbGelInstructionByGelCacheIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* >::iterator endByGelCacheIdx = indexByGelCacheIdx->end();
			std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* curByGelCacheIdx = NULL;
		...
		if( indexByChainIdx != NULL ) {
			std::map< cfcore::CFGenKbGelInstructionByChainIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* >::iterator iterByChainIdx = indexByChainIdx->begin();
			std::map< cfcore::CFGenKbGelInstructionByChainIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* >::iterator endByChainIdx = indexByChainIdx->end();
			std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* curByChainIdx = NULL;
		...
		if( indexByRemainderIdx != NULL ) {
			std::map< cfcore::CFGenKbGelPopByRemainderIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* >::iterator iterByRemainderIdx = indexByRemainderIdx->begin();
			std::map< cfcore::CFGenKbGelPopByRemainderIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* >::iterator endByRemainderIdx = indexByRemainderIdx->end();
			std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* curByRemainderIdx = NULL;
	...
	cfcore::ICFGenKbGelPopObj* CFGenKbGelPopTableObj::newInstance() {
		cfcore::ICFGenKbGelPopObj* inst = dynamic_cast<cfcore::ICFGenKbGelPopObj*>( new CFGenKbGelPopObj( schema ) );
		return( inst );
	}

	cfcore::ICFGenKbGelPopEditObj* CFGenKbGelPopTableObj::newEditInstance( cfcore::ICFGenKbGelPopObj* orig ) {
		cfcore::ICFGenKbGelPopEditObj* edit = dynamic_cast<cfcore::ICFGenKbGelPopEditObj*>( new CFGenKbGelPopEditObj( orig ));
		return( edit );
	}

	cfcore::ICFGenKbGelPopObj* CFGenKbGelPopTableObj::realizeGelPop( cfcore::ICFGenKbGelPopObj* Obj ) {
		static const std::string S_ProcName( "realizeGelPop" );
		...
		cfcore::ICFGenKbGelPopObj* obj = Obj;
		cfcore::ICFGenKbGelPopObj* existingObj = NULL;
		cfcore::CFGenKbGelInstructionPKey* pkey = obj->getPKey();
		cfcore::ICFGenKbGelPopObj* keepObj = NULL;
		...
		if( searchMembers != members->end() ) {
			...
			if( indexByTenantIdx != NULL ) {
				...
				if( searchTenantIdx != indexByTenantIdx->end() ) {
					std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* mapTenantIdx = searchTenantIdx->second;
			...
			if( indexByGelCacheIdx != NULL ) {
				...
				if( searchGelCacheIdx != indexByGelCacheIdx->end() ) {
					std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* mapGelCacheIdx = searchGelCacheIdx->second;
			...
			if( indexByChainIdx != NULL ) {
				...
				if( searchChainIdx != indexByChainIdx->end() ) {
					std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* mapChainIdx = searchChainIdx->second;
					if( mapChainIdx != NULL ) {
			...
			if( indexByRemainderIdx != NULL ) {
				...
				if( searchRemainderIdx != indexByRemainderIdx->end() ) {
					std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* mapRemainderIdx = searchRemainderIdx->second;
			...
			keepObj = dynamic_cast<cfcore::ICFGenKbGelPopObj*>( dynamic_cast<cfcore::CFGenKbGelInstructionTableObj*>( schema->getGelInstructionTableObj() )->realizeGelInstruction( Obj ) );
			if( keepObj == NULL ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
					S_ProcName,
					0,
			...
			if( indexByTenantIdx != NULL ) {
				...
				cfcore::ICFGenKbGelPopObj* aTenantIdxObj =
					dynamic_cast<cfcore::ICFGenKbGelPopObj*>( keepObj );
				...
				if( searchIndexByTenantIdx != indexByTenantIdx->end() ) {
					std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* mapTenantIdx = searchIndexByTenantIdx->second;
					if( mapTenantIdx != NULL ) {
						mapTenantIdx->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj* >::value_type( *(keepObj->getPKey()), aTenantIdxObj ) );
					}
				}
			}
			...
			if( indexByGelCacheIdx != NULL ) {
				...
				cfcore::ICFGenKbGelPopObj* aGelCacheIdxObj =
					dynamic_cast<cfcore::ICFGenKbGelPopObj*>( keepObj );
				...
				if( searchIndexByGelCacheIdx != indexByGelCacheIdx->end() ) {
					std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* mapGelCacheIdx = searchIndexByGelCacheIdx->second;
					if( mapGelCacheIdx != NULL ) {
						mapGelCacheIdx->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj* >::value_type( *(keepObj->getPKey()), aGelCacheIdxObj ) );
					}
				}
			}
			...
			if( indexByChainIdx != NULL ) {
				...
				cfcore::ICFGenKbGelPopObj* aChainIdxObj =
					dynamic_cast<cfcore::ICFGenKbGelPopObj*>( keepObj );
				...
				if( searchIndexByChainIdx != indexByChainIdx->end() ) {
					std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* mapChainIdx = searchIndexByChainIdx->second;
					if( mapChainIdx != NULL ) {
						mapChainIdx->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj* >::value_type( *(keepObj->getPKey()), aChainIdxObj ) );
					}
				}
			}
			...
			if( indexByRemainderIdx != NULL ) {
				...
				cfcore::ICFGenKbGelPopObj* aRemainderIdxObj =
					dynamic_cast<cfcore::ICFGenKbGelPopObj*>( keepObj );
				...
				if( searchIndexByRemainderIdx != indexByRemainderIdx->end() ) {
					std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* mapRemainderIdx = searchIndexByRemainderIdx->second;
					if( mapRemainderIdx != NULL ) {
						mapRemainderIdx->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj* >::value_type( *(keepObj->getPKey()), aRemainderIdxObj ) );
					}
				}
			}

			if( allGelPop != NULL ) {
				allGelPop->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj* >::value_type( *(keepObj->getPKey()), keepObj ) );
			}
		}
		else {
			keepObj = obj;
			...
			members->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj* >::value_type( *(keepObj->getPKey()), keepObj ) );
	...
	void CFGenKbGelPopTableObj::deepDisposeByPIdx( const int64_t TenantId,
			const int64_t GelCacheId,
			const int64_t GelInstId ) {
		static const std::string S_ProcName( "deepDisposeByPIdx" );
		std::vector<cfcore::ICFGenKbGelPopObj*> list;
		cfcore::ICFGenKbGelPopObj* existingObj = readCachedGelPopByPIdx( TenantId,
				GelCacheId,
				GelInstId );
		if( existingObj != NULL ) {
			list.push_back( existingObj );
		}
		cfcore::ICFGenKbGelPopObj* cur = NULL;
		...
		while( listIter != listEnd ) {
			...
			if( cur != NULL ) {
				...
				if( classCode == cfcore::CFGenKbGelPopBuff::CLASS_CODE ) {
	...
	void CFGenKbGelPopTableObj::deepDisposeByTenantIdx( const int64_t TenantId ) {
		static const std::string S_ProcName( "deepDisposeByTenantIdx" );
		std::vector<cfcore::ICFGenKbGelPopObj*> list;
		std::vector<cfcore::ICFGenKbGelPopObj*> matchesFound = readCachedGelPopByTenantIdx( TenantId );
	...
	void CFGenKbGelPopTableObj::deepDisposeByGelCacheIdx( const int64_t TenantId,
			const int64_t GelCacheId ) {
		static const std::string S_ProcName( "deepDisposeByGelCacheIdx" );
		std::vector<cfcore::ICFGenKbGelPopObj*> list;
		std::vector<cfcore::ICFGenKbGelPopObj*> matchesFound = readCachedGelPopByGelCacheIdx( TenantId,
				GelCacheId );
		auto iterMatches = matchesFound.begin();
		auto endMatches = matchesFound.end();
		while( iterMatches != endMatches ) {
	...
	void CFGenKbGelPopTableObj::deepDisposeByChainIdx( const int64_t* ChainInstTenantId,
			const int64_t* ChainInstGelCacheId,
			const int64_t* ChainInstGelInstId ) {
		static const std::string S_ProcName( "deepDisposeByChainIdx" );
		std::vector<cfcore::ICFGenKbGelPopObj*> list;
		std::vector<cfcore::ICFGenKbGelPopObj*> matchesFound = readCachedGelPopByChainIdx( ChainInstTenantId,
				ChainInstGelCacheId,
				ChainInstGelInstId );
		auto iterMatches = matchesFound.begin();
		auto endMatches = matchesFound.end();
	...
	void CFGenKbGelPopTableObj::reallyDeepDisposeGelPop( cfcore::ICFGenKbGelPopObj* Obj ) {
		static const std::string S_ProcName( "reallyDeepDisposeGelPop" );
	...
	void CFGenKbGelPopTableObj::deepDisposeByRemainderIdx( const int64_t* RemainderTenantId,
		...
		std::vector<cfcore::ICFGenKbGelPopObj*> list;
		std::vector<cfcore::ICFGenKbGelPopObj*> matchesFound = readCachedGelPopByRemainderIdx( RemainderTenantId,
				RemainderGelCacheId,
				RemainderInstId );
		auto iterMatches = matchesFound.begin();
		auto endMatches = matchesFound.end();
	...
	cfcore::ICFGenKbGelPopObj* CFGenKbGelPopTableObj::createGelPop( cfcore::ICFGenKbGelPopEditObj* Obj ) {
		static const std::string S_ProcName( "createGelPop" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Cloneable( "cloneable" );
		static const std::string S_ClonedBuff( "clonedbuff" );
		CFLIB_EXCEPTION_DECLINFO
		cfcore::ICFGenKbGelPopObj* obj = dynamic_cast<cfcore::ICFGenKbGelPopObj*>( Obj->getOrig() );
		try {
			cfcore::CFGenKbGelPopBuff* buff = dynamic_cast<cfcore::CFGenKbGelPopBuff*>( Obj->getBuff()->clone() );
			// C++18 version of create returns a new buffer instance and takes over ownership of the passed-in buffer
			// MSS TODO WORKING The xmsg client will need to return the buffer instance created by processing
			// the response message, while xmsg rqst will have to delete the backing store instance
			// it receives after preparing the reply message so that memory doesn't leak on every request.
			cflib::ICFLibCloneableObj* cloneable = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelPop()->createGelPop(
			...
			Obj->endEdit();
			obj->setBuff( dynamic_cast<cfcore::CFGenKbGelPopBuff*>( cloneable ) );
			obj = dynamic_cast<cfcore::ICFGenKbGelPopObj*>( obj->realize() );
			if( obj == NULL ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
					S_ProcName,
					0,
	...
	cfcore::ICFGenKbGelPopObj* CFGenKbGelPopTableObj::readGelPop( cfcore::CFGenKbGelInstructionPKey* pkey, bool forceRead ) {
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		static const std::string S_ProcName( "readGelPop" );
		cfcore::ICFGenKbGelPopObj* obj = NULL;
		cfcore::ICFGenKbGelPopObj* realized = NULL;
		if( ! forceRead ) {
			auto searchMembers = members->find( *pkey );
			if( searchMembers != members->end() ) {
		...
		if( forceRead || ( obj == NULL ) ) {
			cfcore::CFGenKbGelPopBuff* readBuff = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelPop()->readDerivedByPIdx( schema->getAuthorization(),
			pkey->getRequiredTenantId(),
			pkey->getRequiredGelCacheId(),
			pkey->getRequiredGelInstId() );
			if( readBuff != NULL ) {
			obj = dynamic_cast<cfcore::ICFGenKbGelPopObj*>( dynamic_cast<cfcore::CFGenKbGelInstructionTableObj*>( schema->getGelInstructionTableObj() )->constructByClassCode( readBuff->getClassCode() ) );
				obj->setBuff( readBuff );
				realized = dynamic_cast<cfcore::ICFGenKbGelPopObj*>( obj->realize() );
				if( realized == NULL ) {
					throw cflib::CFLibNullArgumentException( CLASS_NAME,
						S_ProcName,
						0,
	...
	cfcore::ICFGenKbGelPopObj* CFGenKbGelPopTableObj::lockGelPop( cfcore::CFGenKbGelInstructionPKey* pkey ) {
		static const std::string S_ProcName( "lockGelPop" );
		cfcore::ICFGenKbGelPopObj* locked = NULL;
		cfcore::CFGenKbGelPopBuff* lockBuff = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelPop()->lockDerived( schema->getAuthorization(), pkey );
		if( lockBuff != NULL ) {
			locked = dynamic_cast<cfcore::ICFGenKbGelPopObj*>( dynamic_cast<cfcore::CFGenKbGelInstructionTableObj*>( schema->getGelInstructionTableObj() )->constructByClassCode( lockBuff->getClassCode() ) );
			locked->setBuff( lockBuff );
			locked = dynamic_cast<cfcore::ICFGenKbGelPopObj*>( locked->realize() );
		}
		else {
			return( NULL );
		}
	...
	std::vector<cfcore::ICFGenKbGelPopObj*> CFGenKbGelPopTableObj::readAllGelPop( bool forceRead ) {
		static const std::string S_ProcName( "readAllGelPop" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Realized( "realized" );
		CFLIB_EXCEPTION_DECLINFO
		cfcore::ICFGenKbGelPopObj* realized = NULL;
		if( forceRead || ( allGelPop == NULL ) ) {
			std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* map = new std::map<cfcore::CFGenKbGelInstructionPKey,cfcore::ICFGenKbGelPopObj*>();
			allGelPop = map;
			std::TCFLibOwningVector<cfcore::CFGenKbGelPopBuff*> buffList = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelPop()->readAllDerived( schema->getAuthorization() );
			cfcore::CFGenKbGelPopBuff* buff = NULL;
			cfcore::ICFGenKbGelPopObj* obj = NULL;
			try {
				for( size_t idx = 0; idx < buffList.size(); idx ++ ) {
					buff = buffList[ idx ];
					buffList[ idx ] = NULL;
				obj = dynamic_cast<cfcore::ICFGenKbGelPopObj*>( dynamic_cast<cfcore::CFGenKbGelInstructionTableObj*>( schema->getGelInstructionTableObj() )->constructByClassCode( buff->getClassCode() ) );
					obj->setBuff( buff );
					realized = dynamic_cast<cfcore::ICFGenKbGelPopObj*>( obj->realize() );
					if( realized == NULL ) {
						throw cflib::CFLibNullArgumentException( CLASS_NAME,
					...
					allGelPop->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj* >::value_type( *(realized->getPKey()), realized ) );
					// No need to delete obj -- realize() auto-destructs the instance it decided to discard
					obj = NULL;
				}
			}
		...
		size_t len = allGelPop->size();
		std::vector<cfcore::ICFGenKbGelPopObj*> arr;
		auto valIter = allGelPop->begin();
		size_t idx = 0;
		while( valIter != allGelPop->end() ) {
	...
	cfcore::ICFGenKbGelPopObj* CFGenKbGelPopTableObj::readGelPopByPIdx( const int64_t TenantId,
			const int64_t GelCacheId,
			const int64_t GelInstId, bool forceRead )
	{
		static const std::string S_ProcName( "readGelPopByPIdx" );
		static const std::string S_Realized( "realized" );
		cfcore::CFGenKbGelInstructionPKey pkey;
		pkey.setRequiredTenantId( TenantId );
		pkey.setRequiredGelCacheId( GelCacheId );
		pkey.setRequiredGelInstId( GelInstId );
		cfcore::ICFGenKbGelPopObj* obj = readGelPop( &pkey, forceRead );
		return( obj );
	}

	std::vector<cfcore::ICFGenKbGelPopObj*> CFGenKbGelPopTableObj::readGelPopByTenantIdx( const int64_t TenantId,
		bool forceRead )
	{
		static const std::string S_ProcName( "readGelPopByTenantIdx" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		cfcore::CFGenKbGelInstructionByTenantIdxKey key;
		key.setRequiredTenantId( TenantId );
		std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* dict;
		std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>* oldDict;
		if( indexByTenantIdx == NULL ) {
			indexByTenantIdx = new std::map< cfcore::CFGenKbGelInstructionByTenantIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>*>();
		}
		...
		if( ( oldDict != NULL ) && ( ! forceRead ) ) {
			...
			dict = new std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>();
			...
			for( size_t idx = 0; idx < buffList.size(); idx ++ ) {
				buff = buffList[ idx ];
				buffList[ idx ] = NULL;
				obj = dynamic_cast<cfcore::ICFGenKbGelPopObj*>(
				...
				obj->setBuff( buff );
				cfcore::ICFGenKbGelPopObj* realized = dynamic_cast<cfcore::ICFGenKbGelPopObj*>( obj->realize() );
				if( realized == NULL ) {
					throw cflib::CFLibNullArgumentException( CLASS_NAME,
						S_ProcName,
						0,
				...
				dict->insert( std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj* >::value_type( *(realized->getPKey()), realized ) );
				// No need to delete obj -- realize() auto-destructs the instance it decided to discard
				obj = NULL;
			}
			if( oldDict != NULL ) {
			...
			indexByTenantIdx->insert( std::map< cfcore::CFGenKbGelInstructionByTenantIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj* >* >::value_type( key, dict ) );
		}
	...
	std::vector<cfcore::ICFGenKbGelPopObj*> CFGenKbGelPopTableObj::readGelPopByGelCacheIdx( const int64_t TenantId,
			const int64_t GelCacheId,
		bool forceRead )
	{
		static const std::string S_ProcName( "readGelPopByGelCacheIdx" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
	...
	std::vector<cfcore::ICFGenKbGelPopObj*> CFGenKbGelPopTableObj::readGelPopByChainIdx( const int64_t* ChainInstTenantId,
			const int64_t* ChainInstGelCacheId,
			const int64_t* ChainInstGelInstId,
		bool forceRead )
	{
		static const std::string S_ProcName( "readGelPopByChainIdx" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
	...
	std::vector<cfcore::ICFGenKbGelPopObj*> CFGenKbGelPopTableObj::readGelPopByRemainderIdx( const int64_t* RemainderTenantId,
			const int64_t RemainderGelCacheId,
			const int64_t* RemainderInstId,
		bool forceRead )
	{
		static const std::string S_ProcName( "readGelPopByRemainderIdx" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		...
		if( ( oldDict != NULL ) && ( ! forceRead ) ) {
			...
			dict = new std::map<cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj*>();
			cfcore::ICFGenKbGelPopObj* obj;
			std::TCFLibOwningVector<cfcore::CFGenKbGelPopBuff*> buffList = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelPop()->readDerivedByRemainderIdx( schema->getAuthorization(),
				RemainderTenantId,
				RemainderGelCacheId,
				RemainderInstId );
			...
			indexByRemainderIdx->insert( std::map< cfcore::CFGenKbGelPopByRemainderIdxKey,
				std::map< cfcore::CFGenKbGelInstructionPKey, cfcore::ICFGenKbGelPopObj* >* >::value_type( key, dict ) );
		}
		size_t len = dict->size();
		std::vector<cfcore::ICFGenKbGelPopObj*> arr;
	...
	cfcore::ICFGenKbGelPopObj* CFGenKbGelPopTableObj::readCachedGelPop( cfcore::CFGenKbGelInstructionPKey* pkey ) {
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		static const std::string S_ProcName( "readGelPop" );
		cfcore::ICFGenKbGelPopObj* obj = NULL;
	...
	cfcore::ICFGenKbGelPopObj* CFGenKbGelPopTableObj::readCachedGelPopByPIdx( const int64_t TenantId,
			const int64_t GelCacheId,
			const int64_t GelInstId )
	{
		static const std::string S_ProcName( "readCachedGelPopByPIdx" );
		static const std::string S_Realized( "realized" );
		cfcore::CFGenKbGelInstructionPKey pkey;
		pkey.setRequiredTenantId( TenantId );
		pkey.setRequiredGelCacheId( GelCacheId );
		pkey.setRequiredGelInstId( GelInstId );
		cfcore::ICFGenKbGelPopObj* obj = readCachedGelPop( &pkey );
		return( obj );
	}

	std::vector<cfcore::ICFGenKbGelPopObj*> CFGenKbGelPopTableObj::readCachedGelPopByTenantIdx( const int64_t TenantId )
	{
		static const std::string S_ProcName( "readCachedGelPopByTenantIdx" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		std::vector<cfcore::ICFGenKbGelPopObj*> arr;
	...
	std::vector<cfcore::ICFGenKbGelPopObj*> CFGenKbGelPopTableObj::readCachedGelPopByGelCacheIdx( const int64_t TenantId,
			const int64_t GelCacheId )
	{
		static const std::string S_ProcName( "readCachedGelPopByGelCacheIdx" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		std::vector<cfcore::ICFGenKbGelPopObj*> arr;
	...
	std::vector<cfcore::ICFGenKbGelPopObj*> CFGenKbGelPopTableObj::readCachedGelPopByChainIdx( const int64_t* ChainInstTenantId,
			const int64_t* ChainInstGelCacheId,
			const int64_t* ChainInstGelInstId )
	{
		static const std::string S_ProcName( "readCachedGelPopByChainIdx" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		std::vector<cfcore::ICFGenKbGelPopObj*> arr;
	...
	std::vector<cfcore::ICFGenKbGelPopObj*> CFGenKbGelPopTableObj::readCachedGelPopByRemainderIdx( const int64_t* RemainderTenantId,
			const int64_t RemainderGelCacheId,
			const int64_t* RemainderInstId )
	{
		static const std::string S_ProcName( "readCachedGelPopByRemainderIdx" );
		static const std::string S_Idx( "idx" );
		static const std::string S_Obj( "obj" );
		static const std::string S_Realized( "realized" );
		std::vector<cfcore::ICFGenKbGelPopObj*> arr;
		...
		if( searchIndexByRemainderIdx != indexByRemainderIdx->end() ) {
			...
			for( auto iterMembers = members->begin(); iterMembers != members->end(); iterMembers ++ ) {
				obj = iterMembers->second;
				if( obj != NULL ) {
					if( *(dynamic_cast<cfcore::CFGenKbGelPopBuff*>( obj->getBuff() ) ) == key ) {
						arr.push_back( obj );
					}
				}
			}
	...
	cfcore::ICFGenKbGelPopObj* CFGenKbGelPopTableObj::updateGelPop( cfcore::ICFGenKbGelPopEditObj* Obj ) {
		...
		cfcore::ICFGenKbGelPopObj* obj = dynamic_cast<cfcore::ICFGenKbGelPopObj*>( Obj->getOrig() );
		try {
			cfcore::CFGenKbGelPopBuff* updated = dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelPop()->updateGelPop( schema->getAuthorization(),
				dynamic_cast<cfcore::CFGenKbGelPopBuff*>( Obj->getGelPopBuff()->clone() ) );
			if( updated == NULL ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
					S_ProcName,
					0,
			...
			obj = dynamic_cast<cfcore::ICFGenKbGelPopObj*>( dynamic_cast<cfcore::CFGenKbGelPopTableObj*>( schema->getGelPopTableObj() )->newInstance() );
			obj->setBuff( updated );
			obj = dynamic_cast<cfcore::ICFGenKbGelPopObj*>( obj->realize() );
			if( obj == NULL ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
	...
	void CFGenKbGelPopTableObj::deleteGelPop( cfcore::ICFGenKbGelPopEditObj* Obj ) {
		cfcore::ICFGenKbGelPopObj* obj = Obj;
		dynamic_cast<ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelPop()->deleteGelPop( schema->getAuthorization(),
			obj->getGelPopBuff() );
	...
	void CFGenKbGelPopTableObj::deleteGelPopByPIdx( const int64_t TenantId,
			const int64_t GelCacheId,
			const int64_t GelInstId )
	{
		cfcore::CFGenKbGelInstructionPKey pkey;
		...
		pkey.setRequiredGelCacheId( GelCacheId );
		pkey.setRequiredGelInstId( GelInstId );
		cfcore::ICFGenKbGelPopObj* obj = readGelPop( &pkey, true );
		if( obj != NULL ) {
			cfcore::ICFGenKbGelPopEditObj* editObj = dynamic_cast<cfcore::ICFGenKbGelPopEditObj*>( obj->getEdit() );
			if( editObj == NULL ) {
				editObj = dynamic_cast<cfcore::ICFGenKbGelPopEditObj*>( obj->beginEdit() );
			}
			if( editObj != NULL ) {
	...
	void CFGenKbGelPopTableObj::deleteGelPopByTenantIdx( const int64_t TenantId )
	{
		dynamic_cast<cfcore::ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelPop()->deleteGelPopByTenantIdx( schema->getAuthorization(),
				TenantId );
		deepDisposeByTenantIdx( TenantId );
	}

	void CFGenKbGelPopTableObj::deleteGelPopByGelCacheIdx( const int64_t TenantId,
			const int64_t GelCacheId )
	{
		dynamic_cast<cfcore::ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelPop()->deleteGelPopByGelCacheIdx( schema->getAuthorization(),
				TenantId,
				GelCacheId );
	...
	void CFGenKbGelPopTableObj::deleteGelPopByChainIdx( const int64_t* ChainInstTenantId,
			const int64_t* ChainInstGelCacheId,
			const int64_t* ChainInstGelInstId )
	{
		dynamic_cast<cfcore::ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelPop()->deleteGelPopByChainIdx( schema->getAuthorization(),
				ChainInstTenantId,
				ChainInstGelCacheId,
				ChainInstGelInstId );
	...
	void CFGenKbGelPopTableObj::deleteGelPopByRemainderIdx( const int64_t* RemainderTenantId,
			const int64_t RemainderGelCacheId,
			const int64_t* RemainderInstId )
	{
		dynamic_cast<cfcore::ICFGenKbSchema*>( schema->getBackingStore() )->getTableGelPop()->deleteGelPopByRemainderIdx( schema->getAuthorization(),
				RemainderTenantId,
				RemainderGelCacheId,
				RemainderInstId );
	...
	void CFGenKbGelPopTableObj::reallyDetachFromIndexesGelPop( cfcore::ICFGenKbGelPopObj* Obj ) {
		static const std::string S_ProcName( "reallyDetachFromIndexesGelPop" );
		static const std::string S_Obj( "Obj" );
		static const std::string S_ExistingObj( "ExistingObj" );
		if( Obj == NULL ) {
			throw cflib::CFLibNullArgumentException( CLASS_NAME, S_ProcName, 1, S_Obj );
		...
		auto searchMembers = members->find( *pkey );
		if( searchMembers != members->end() ) {
			cfcore::ICFGenKbGelPopObj* existingObj = searchMembers->second;
			if( existingObj == NULL ) {
				throw cflib::CFLibNullArgumentException( CLASS_NAME,
					S_ProcName,
					0,
